...
export default async (request)=>{
  ...
  const payload={
    ...
    instructions:
      "You are GrowthWise Vehicle Scout for a small independent dealership with an in-house repair shop. Search CURRENT public vehicle listings and surface plausible acquisition RESEARCH LEADS. Freshness matters: prefer evidence that is clearly current or recently updated, and explicitly classify every lead as Current, Recent, Stale, or Unknown based on the freshest evidence you can see. Current means evidence within about 7 days, Recent means roughly 8-30 days, Stale means older than 30 days, and Unknown means the age cannot be established. Do not invent freshness dates. Do not invent vehicles, prices, mileage, locations, VINs, title status, condition, accident history, repair needs or completed-sale prices. Use public listing evidence only. Return useful leads even when the exact listing page cannot yet be verified; a separate verification step handles that. Clearly lower evidence quality when details are incomplete or stale. Return only the requested structured data.",
    ...
    text:{verbosity:"low",format:{type:"json_schema",name:"growthwise_vehicle_scout_background",strict:true,schema:{type:"object",additionalProperties:false,properties:{search_summary:{type:"string"},confidence:{type:"string",enum:["Low","Medium","High"]},opportunities:{type:"array",maxItems:5,items:{type:"object",additionalProperties:false,properties:{year:{type:"number"},make:{type:"string"},model:{type:"string"},trim:{type:"string"},source_type:{type:"string"},listing_url:{type:"string"},location:{type:"string"},asking_price:{type:"number"},mileage:{type:"number"},retail_low:{type:"number"},retail_mid:{type:"number"},retail_high:{type:"number"},estimated_fees:{type:"number"},estimated_transport:{type:"number"},estimated_recon:{type:"number"},estimated_labor:{type:"number"},score:{type:"number"},evidence_quality:{type:"string",enum:["Low","Medium","High"]},freshness_status:{type:"string",enum:["Current","Recent","Stale","Unknown"]},freshness_note:{type:"string"},why_fit:{type:"string"},caveats:{type:"array",items:{type:"string"}}},required:["year","make","model","trim","source_type","listing_url","location","asking_price","mileage","retail_low","retail_mid","retail_high","estimated_fees","estimated_transport","estimated_recon","estimated_labor","score","evidence_quality","freshness_status","freshness_note","why_fit","caveats"]}}},required:["search_summary","confidence","opportunities"]}}}
  };
