...
function normalizeVersions(configuration, { encryption = false, code }) {
  ...
  return entries.map((entry) => {
    ...
    // A binding version is later prefixed to a 43-character HMAC and used as
    // one validated AAD segment, so keep every version identifier bounded.
    if (id.length > 64) fail(code);
...
function bindingValue(version, accountId) {
  requireSegment(accountId, "INVALID_ACCOUNT_CONTEXT");
  return `${version.id}.${createHmac("sha256", version.key).update(accountId, "utf8").digest("base64url")}`;
}
...
function aad(businessId, accountBindingKey) {
  ...
  return Buffer.from(
    `growthwise-instagram-database\ncredential-v1\n${businessId}\ninstagram\n${accountBindingKey}`,
    "utf8",
  );
}

export function createInstagramCrypto({
  stateSecrets,
  bindingSecrets,
  credentialKeys,
  randomBytesImpl = randomBytes,
}) {
  const states = normalizeVersions(stateSecrets, { code: "INVALID_STATE_CONFIGURATION" });
  const bindings = normalizeVersions(bindingSecrets, { code: "INVALID_BINDING_CONFIGURATION" });
  ...
  return {
    ...
    transactionKey(state) {
      return verifyState(state).nonceHash;
    },
    ...
    encryptCredential({ businessId, accountId, payload }) {
      ...
      try {
        ...
        fail("INVALID_CREDENTIAL_PAYLOAD");
      }
    ...
    decryptCredential({ businessId, accountId, accountBindingKey: storedBindingKey, encryptedToken }) {
      ...
      try {
        ...
        if (typeof encryptedToken.ciphertext !== "string" || !/^[A-Za-z0-9_-]+$/.test(encryptedToken.ciphertext))
          fail("CREDENTIAL_DECRYPT_FAILED");
        const combined = Buffer.from(encryptedToken.ciphertext, "base64url");
        if (combined.length <= GCM_TAG_BYTES || combined.toString("base64url") !== encryptedToken.ciphertext)
          fail("CREDENTIAL_DECRYPT_FAILED");
        ...
        for (const accountBindingKey of aadBindingKeys) {
          try {
            ...
            // Binding-secret rotation requires trying each configured AAD value.
          }
        }
        fail("CREDENTIAL_DECRYPT_FAILED");
      } catch (error) {
