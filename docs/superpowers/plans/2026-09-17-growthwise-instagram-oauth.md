...
**Tech Stack:** Node.js ES modules, built-in `node:test` / `node:assert/strict` / `node:crypto`, Netlify Functions, `@netlify/database`, PostgreSQL migrations under `netlify/database/migrations/<number>_<lowercase-slug>/migration.sql`, JSON client configuration, and the existing HTML/CSS/JavaScript application. Existing Publishing Core may continue using Netlify Blobs.
...
- `tests/integration/instagram-database.test.mjs` (runs only against an already-migrated isolated Deploy Preview database branch; it never applies DDL or drops application objects)
...
**Files:**
- Create: `netlify/database/migrations/<number>_<lowercase-slug>/migration.sql`
- Create: `netlify/functions/_instagram-store.mjs`
- Create: `tests/publishing/instagram-store.test.mjs`
- Modify: `package.json` and repository lockfile when it can be generated by npm
...
git diff --check
git add package.json netlify/database/migrations netlify/functions/_instagram-store.mjs tests/publishing/instagram-store.test.mjs
...
**Files:**
- Modify: `netlify/database/migrations/<number>_<lowercase-slug>/migration.sql`
...
The integration command is **PASS** only against an automatically migrated, isolated Netlify Deploy Preview database branch. It inspects the installed schema, uses uniquely prefixed synthetic rows, and deletes only those rows; it never applies migration SQL or drops application objects. If that preview context is unavailable, report **NOT TESTABLE** and retain it as a pre-merge/deployment acceptance requirement; do not use production and do not fall back to Blobs.
