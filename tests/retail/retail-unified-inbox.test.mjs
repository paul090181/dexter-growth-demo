...
const ingestCore = await readFile(new URL("../../netlify/functions/_retail-lead-ingest.mjs", import.meta.url), "utf8");
...
test("unified retail inbox stores normalized source metadata and de-duplicates provider messages", () => {
  ...
  assert.match(ingest, /ingestRetailLead/);
  assert.match(ingestCore, /external_message_id/);
  assert.match(ingestCore, /duplicate: true/);
  assert.match(ingestCore, /source_metadata/);
});
...
test("unified retail inbox supports Instagram Facebook email website SMS and manual sources", () => {
  for (const source of ["instagram","facebook","email","website","sms","manual"]) {
    ...
    assert.match(ingestCore, new RegExp(source));
  }
});
